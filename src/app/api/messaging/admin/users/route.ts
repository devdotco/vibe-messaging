  ...
  try {
    ...
    if (apiKey) {
      ...
      await sgMail.send({
        ...
        to: email,
        subject: `You've been invited to erp.io Messaging`,
        html: `<p>Hi ${name},</p><p>You've been invited to join erp.io Messaging.</p><p><a href="${magicLink}" style="background:#2563eb;color:white;padding:10px 20px;border-radius:6px;text-decoration:none;display:inline-block">Accept Invite &amp; Sign In</a></p><p>Or copy this link: ${magicLink}</p>`,
      });
    }
  } catch (err) {
